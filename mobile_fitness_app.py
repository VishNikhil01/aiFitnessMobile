...
class Transformer(VideoTransformerBase):
    ...
    def recv(self, frame):
        ...
        for det in preds:
            ...
            # collect 33 landmarks into dict
            pts = {p.name:[lm[p].x,lm[p].y] for p in mp.solutions.pose.PoseLandmark}

            # compute 11 angles
            neck = (calculateAngle(pts['LEFT_SHOULDER'],pts['NOSE'],pts['LEFT_HIP'])
                  +calculateAngle(pts['RIGHT_SHOULDER'],pts['NOSE'],pts['RIGHT_HIP']))/2
            ...
            # sidebar print
            st.sidebar.text(f"Neck: {neck:.1f}°");st.sidebar.text(f"L Elbow: {le:.1f}°")
            st.sidebar.text(f"R Elbow: {re:.1f}°");st.sidebar.text(f"L Shoulder: {ls:.1f}°")
            st.sidebar.text(f"R Shoulder: {rs:.1f}°");st.sidebar.text(f"L Hip: {lh:.1f}°")
            st.sidebar.text(f"R Hip: {rh:.1f}°");st.sidebar.text(f"L Knee: {lk:.1f}°")
            st.sidebar.text(f"R Knee: {rk:.1f}°");st.sidebar.text(f"L Ankle: {la:.1f}°")
            st.sidebar.text(f"R Ankle: {ra:.1f}°")
            ...
            if "down" in pred:
                ...
                if tag!="correct" and now-self.last_alert>3:
                    ...
                    st.error(msg)
                    if snd: pygame.mixer.music.load(snd); pygame.mixer.music.play()
